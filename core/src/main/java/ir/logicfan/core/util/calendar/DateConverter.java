 ...
 * <p>
 * This class is a main class of calendar as it converts jalali to gregorian and gregorian to jalali.
...
public class DateConverter {
    ...
    private double persianMonthSelectedIndex;
    private double persianDayValue;
     ...
     *
     * @param jalaliCalendar will be converted to suitable form of gregorian date.
     * @return {@link GregorianCalendar} gregorian date will be returned.
     */
    public GregorianCalendar jalaliToGregorian(JalaliCalendar jalaliCalendar) {
        return jalaliToGregorian(jalaliCalendar.getYear(), jalaliCalendar.getMonth().getValue(),
                jalaliCalendar.getDay(), jalaliCalendar.getClock());
    }
     ...
     * @param month is {@link MonthPersian} which is similar to java8's {@link Month}
     ...
     */
    public GregorianCalendar jalaliToGregorian(int year, MonthPersian month, int day) {
     ...
     * @see JalaliCalendar
     */
    public GregorianCalendar jalaliToGregorian(int year, int month, int day) {
        return jalaliToGregorian(year, month, day, new Clock());
    }

    public GregorianCalendar jalaliToGregorian(int year, int month, int day, Clock clock) {
        validateDateValues(year, month, day);

        persianYearValue = year;
        persianMonthSelectedIndex = month - 1;
        persianDayValue = day;

        setJulian(persianaToJd((persianYearValue), persianMonthSelectedIndex + 1, persianDayValue) + 0.5);

        return new GregorianCalendar(
                (int) gregorianYearValue,
                (int) gregorianMonthSelectedIndex,
                (int) gregorianDayValue,
     ...
     * @see JalaliCalendar
     */
    public JalaliCalendar gregorianToJalali(int year, int month, int day) {
        validateDateValues(year, month, day);
        return gregorianToJalali(new GregorianCalendar(year, month - 1, day));
    }

    public JalaliCalendar gregorianToJalali(GregorianCalendar calendar) {
        updateFromGregorian(
                calendar.get(Calendar.YEAR),
                calendar.get(Calendar.MONTH),
                calendar.get(Calendar.DAY_OF_MONTH));
        int persianYear = (int) persianYearValue;
        int persianMonth = (int) (persianMonthSelectedIndex);
        int persianDay = (int) persianDayValue;
        int hourOfDay = calendar.get(Calendar.HOUR_OF_DAY);
        int minute = calendar.get(Calendar.MINUTE);
        int second = calendar.get(Calendar.SECOND);
        Clock clock = new Clock(hourOfDay, minute, second);
        return new JalaliCalendar(persianYear, persianMonth, persianDay, clock);
    }
    ...
    private void updateFromGregorian(int gregorianYear, int gregorianMonth, int gregorianDay) {
        ...
        persianYearValue = perscal[0];
        persianMonthSelectedIndex = perscal[1];
        persianDayValue = perscal[2];
    }
